...
export const ChatHistory = ({ messages }) => {
  return (
    <div className="chat-history p-4 bg-[#2a2b33] rounded-md">
      {messages.map((msg, index) => (
        <div 
          key={index} 
          className={`mb-2 p-2 rounded ${
            msg.role === 'user' 
              ? 'bg-[#8417ff] text-white self-end' 
              : 'bg-[#1c1d25] text-neutral-300 self-start'
          }`}
        >
          {msg.content}
        </div>
...
//   setChatHistory([...chatHistory, { role: "bot", content: data.response }]);
// };
// ------------------1
// ------------------2

const userInput = "what is education?";
...
const handleChatWithLLM = async () => {
  try {
    ...
    // Update chat history with both user input and LLM response
    ...
    // If you want to also send this as a message in the current chat
  ...
  return (
    <div className="h-[10vh] bg-[#1c1d25] flex justify-center items-center px-8 gap-6 mb-5">
      <div className="flex-1 flex bg-[#2a2b33] rounded-md items-center gap-5 pr-5">
        ...
        <button onClick={handleChatWithLLM}>Chat with LLM</button>
